...
type FinanceTransactionCandidate = {
  id: string;
...
export async function DashboardScreen() {
  try {
    ...
    return (
      <FinanceShell title="Fourteen Cash Pulse" eyebrow="Financial cockpit">
...
export async function TransactionsScreen() {
  try {
    const items = await fetchFinanceJson<FinanceTransaction[]>('/transactions');
...
export async function ReviewScreen() {
  try {
    const [items, candidates] = await Promise.all([
      fetchFinanceJson<FinanceReviewItem[]>('/review-items'),
      fetchFinanceJson<FinanceTransactionCandidate[]>('/transaction-candidates'),
    ]);
    ...
    return (
      <FinanceShell title="Review queue" eyebrow="Candidates and evidence">
...
async function acceptCandidateAction(formData: FormData) {
  ...
  if (typeof candidateId === 'string' && candidateId.length > 0) {
    await postFinanceJson(`/transaction-candidates/${candidateId}/accept`);
  }
  redirect('/review');
}
...
async function updateCandidateAction(formData: FormData) {
  ...
  const candidateId = formData.get('candidateId');
  if (typeof candidateId === 'string' && candidateId.length > 0) {
    await patchFinanceJson(`/transaction-candidates/${candidateId}`, {
...
async function createCandidateFromReviewItemAction(formData: FormData) {
  ...
  await postFinanceJson('/transaction-candidates/from-raw-email', {
...
async function rejectCandidateAction(formData: FormData) {
  ...
  const candidateId = formData.get('candidateId');
  if (typeof candidateId === 'string' && candidateId.length > 0) {
    await postFinanceJson(`/transaction-candidates/${candidateId}/reject`);
  }
  redirect('/review');
}
...
export async function SimpleCollectionScreen({
  ...
  try {
    const items = await fetchFinanceJson<unknown[]>(endpoint);
...
async function updateFixedObligationAction(formData: FormData) {
  ...
  if (typeof id === 'string' && id.length > 0) {
    await patchFinanceJson(`/fixed-obligations/${id}`, {
...
async function createCategoryAction(formData: FormData) {
  'use server';
  await postFinanceJson('/categories', getFormPayload(formData, ['name']));
