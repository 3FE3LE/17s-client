...
import { ErrorBlock, FinanceShell } from '@/components/finance-shell';
import {
  deleteFinanceJson,
  fetchFinanceJson,
  patchFinanceJson,
...
export default async function Page({
  ...
  try {
    const [connections, rawEmails, channels] = await Promise.all([
      fetchFinanceJson<EmailConnection[]>('/email-sources'),
      fetchFinanceJson<RawEmail[]>('/email-sources/raw-emails'),
      fetchFinanceJson<NotificationChannel[]>('/email-sources/notification-channels'),
    ]);
    ...
    return (
      <FinanceShell title="Email ingestion" eyebrow="Candidate discovery and approved sender sync">
                ...
                <div>
                  ...
                  <p className="m-0 mt-1 text-md leading-[1.5] text-muted">
                    {gmailConnection
                      ...
                      : 'Connect Gmail to discover finance emails and approve senders.'}
                  </p>
                </div>
    ...
    return (
      <FinanceShell title="Email sources" eyebrow="Google Gmail ingestion">
...
async function connectGmailAction() {
  ...
  try {
    response = await postFinanceJson<ConnectResponse>('/email-sources/gmail/connect');
  } catch (error) {
...
async function syncGmailAction(formData?: FormData) {
  ...
  try {
    response = await postFinanceJson<SyncResponse>('/email-sources/gmail/sync-recent', {
      searchQuery,
      maxResults,
    });
  } catch (error) {
...
async function syncApprovedChannelsAction(formData?: FormData) {
  ...
  let response: SyncResponse;
  try {
    response = await postFinanceJson<SyncResponse>('/email-sources/gmail/sync-approved-channels', {
      maxResults,
    });
  } catch (error) {
    redirect(`/settings/email-sources?gmail_error=${encodeError(error)}`);
...
async function disableGmailAction() {
  ...
  try {
    await patchFinanceJson('/email-sources/gmail/disable');
  } catch (error) {
    redirect(`/settings/email-sources?gmail_error=${encodeError(error)}`);
  }
...
async function deleteGmailAction() {
  ...
  try {
    await deleteFinanceJson('/email-sources/gmail/connection');
  } catch (error) {
    redirect(`/settings/email-sources?gmail_error=${encodeError(error)}`);
  }
...
async function resetDetectedDataAction() {
  ...
  try {
    await postFinanceJson('/email-sources/reset-detected-data');
  } catch (error) {
    redirect(`/settings/email-sources?gmail_error=${encodeError(error)}`);
  }
...
async function createNotificationChannelAction(formData: FormData) {
  ...
  if (typeof rawEmailId === 'string') {
    await postFinanceJson('/email-sources/notification-channels', {
      rawEmailId,
