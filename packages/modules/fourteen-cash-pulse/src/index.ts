...
export const FourteenCashPulseFeatureFlags = {
  enableOutlookIngestion: 'fourteen-cash-pulse.enable_outlook_ingestion',
  enableReviewQueue: 'fourteen-cash-pulse.enable_review_queue',
} as const;
...
export const FinanceDashboardOverviewSchema = z.object({
  month: z.string(),
...
export type FinanceDashboardOverview = z.infer<typeof FinanceDashboardOverviewSchema>;
...
export interface FinanceApi {
  getOverview: () => Promise<FinanceDashboardOverview>;
  listTransactions: () => Promise<FinanceTransaction[]>;
  listReviewItems: () => Promise<FinanceReviewItem[]>;
...
export function createFinanceApi(baseUrl = '/api/finance'): FinanceApi {
  ...
  return {
    async getOverview() {
      return FinanceDashboardOverviewSchema.parse(await request('/dashboard/overview'));
    },
