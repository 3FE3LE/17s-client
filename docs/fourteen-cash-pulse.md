...
Fourteen Cash Pulse is the finance cockpit product added under the existing 17Suit client monorepo.

## What Was Added

- Web app: `apps/fourteen-cp-web`
- Mobile scaffold: `apps/fourteen-cp-mobile`
- Product module: `packages/modules/fourteen-cash-pulse`
- Landing registration: `apps/landing/app/page.tsx` and `apps/landing/app/[slug]/page.tsx`

The chosen suite name is `Fourteen Cash Pulse` because numbers 1, 2, 4, 5, 6, 7, 8, and 9 already exist in code, 11 is reserved in the roadmap for Eleven Event Heaven, and 14 was available.
...
cd 17s-client
pnpm install
pnpm --filter @17suit/fourteen-cp-web dev
```

The web app runs on port `3003`.
...
- `CLERK_SECRET_KEY`
- `API_BASE_URL`
- `FOURTEEN_CP_WEB_URL` on the server, recommended as `http://localhost:3003`
